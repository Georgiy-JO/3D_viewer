...
Vect3 Normalize(const Vect3& vec) {
    ...
    if (length > 0.0) {
        return Vect3(vec.x / length, vec.y / length, vec.z / length);
    }
    return Vect3(0.0, 1.0, 0.0);
}
...
Mat4 Rotate(Mat4& mat4x4, double angle, const Vect3& vec) {
    const float c = std::cos(angle);
    const float s = std::sin(angle);
    const Vect3 axis = Normalize(vec);
    const float t = 1.0 - c;
...
Mat4::Mat4(const matrix::S21Matrix& other) : m_mat4x4(other) {}
