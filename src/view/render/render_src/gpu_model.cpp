...
void GPU_Model::InitializeModel() {
  ...
  if (!QOpenGLContext::currentContext())
    throw std::runtime_error(
        "InitializeModel called without a current OpenGL context!");
  if (m_vao != 0) return;

  // initialize function pointers for modern OpenGL (from
  // QOpenGLFunctions_3_3_Core)
  initializeOpenGLFunctions();  // it should be initialized already before
  ...
  glBindBuffer(GL_ARRAY_BUFFER,
               m_vbo);  // bind VBO as array buffer (vertex data)
  ...
  // Generate EBO (element/index buffer) for edges and tells what kind of buffer
  // it will work with
...
void GPU_Model::SetModelData(std::shared_ptr<inbound_model::Model3D> model_) {
  ...
  if (m_vao == 0)
    throw std::runtime_error(
        "GPU buffers were not initialized (something went terribly wrong).");
  ...
  /**
   * @note We need do transfor the data from Model3D to save space and store
   * data tightly. We could send Vec3 straight to GPU if Vec3 would be based of
   * float (it's double). Overflow won't happen due to fact that model is
   * normalized already.
   */
  ...
  for (uint32_t i = 0; i < m_VerticesAmount; ++i) {
    ...
    if (v.x > std::numeric_limits<float>::max() ||
      ...
      throw std::runtime_error(
          "Model was not normalized before passing to GPU buffer (Vertex has "
          "value out of range).");
   ...
   *      GL_ARRAY_BUFFER - it tells OpenGL that the buffer we’re working with
   * is a vertex attribute buffer (it is connected to m_vbo). GL_STATIC_DRAW -
   * it tells OpenGL that we won’t be modifying the data in the buffer very
   * often (GL_STREAM_DRAW → changes every frame). We are using GPU side
   * transformations (via shaders): in this case the vertex buffer data stays
   * the same — the GPU just transforms it on the fly for rendering.
   */
...
const GLuint& GPU_Model::GetVertexArrayObject() const { return m_vao; }

size_t GPU_Model::GetEdgesCount() const { return m_EdgesCount; }

uint32_t GPU_Model::GetVerticesAmount() const { return m_VerticesAmount; }
