...
void Capturer<ChildClass>::SetActiveType(int type_) {
  if (type_ < ChildClass::kTypeAmount && type_ >= 0) m_active_type = type_;
}
...
const std::string Capturer<ChildClass>::GetTypeList() const {
  ...
  for (int i = 0, j = 0; i < ChildClass::kTypeAmount; i++) {
    ...
    output += " (";
    for (j = i; j < ChildClass::kTypeAmount &&
                ChildClass::kTypeList[i].type == ChildClass::kTypeList[j].type;
         j++) {
      if (i != j) output += " ";
      output += "*";
...
bool Capturer<ChildClass>::SetFileName(const std::string& file_name_) {
  try {
    if (file_name_ != m_file_name) m_file_name = FixFileName(file_name_);
    return true;
...
template <typename ChildClass>
const std::string Capturer<ChildClass>::FixFileName(
    const std::string& file_name_) const {
  if (file_name_.empty()) throw std::runtime_error("File name is empty!");

  std::string local_file_name = file_name_;
  int file_type = -1;
  for (int i = 0; i < ChildClass::kTypeAmount; i++) {
    if (local_file_name.ends_with(ChildClass::kTypeList[i].tag)) file_type = i;
  }
  if (file_type == -1) {
  ...
  if (std::filesystem::exists(local_file_name)) {
    ...
    while (true) {
      ...
      if (!std::filesystem::exists(test_name)) break;
    }
