...
namespace gui::capture{
    ...
    const std::string ScreenshotMaker::GetDefaultFileName() const{
        return kCaptureLocation+std::string("/screenshot")+kTypeList[m_active_type].tag.data();
    }

    bool ScreenshotMaker::SaveScreenshot(const QPixmap& shot_, const std::string& file_name_){
    ...
    bool ScreenshotMaker::SaveScreenshot(const QPixmap& shot_) const{
        ...
        return shot_.save(QString::fromStdString(m_file_name), QString(kTypeList[file_type].type.data()).toUtf8());
    }
} //gui::capture