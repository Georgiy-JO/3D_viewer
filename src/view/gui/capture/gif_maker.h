...
class GifMaker : public Capturer<GifMaker>{
  ...
  public:
     ...
     * @attention This function ends recording of the current GIF (if there is one).
     * @note This function will use the file_name that was set before (set it before using it).
     */
    bool RecordGif();
    /**
     * @brief Function to start recording GIF.
     * @attention Add following frames with RecordGif(const QPicture& input_pic).
     * @attention After recording all the frames call SaveGif().
     * @note If you set new file_name_ while the gif is still recording old one will be saved
     * and new one will be created. This may damage the currently recording GIF.
     */
    bool RecordGif(const std::string& file_name_);
    /**
     * @brief Function to start recording GIF and add frames to currently recording gif.
     * @attention After recording all the frames call SaveGif().
     * @note If you set new file_name_ while the gif is still recording old one will be saved
     * and new one will be created. This may damage the currently recording GIF. Due to that it's 
     * better to use this one only to start recording, not to add new frames.
     */
