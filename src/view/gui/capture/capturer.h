...
struct FileType
{
    ...
    constexpr FileType(std::string_view type_,std::string_view tag_):type(type_),tag(tag_){}
};

/**
 * @note Child subclasses must have their own kTypeList and kTypeAmount constants, 
 * and override GetDefaultFileName() function.
 */
...
class Capturer {
  public:
    ...
    /**
     * @note The list of types has the folowing structure: type_1 (tag_1);;type_2 (tag_2) etc.
     */
    const std::string GetTypeList() const;

  protected:
    /**
     * @brief Checks the file_name_: if it's empty, are there any files with same name,
     *        does it has tag. 
