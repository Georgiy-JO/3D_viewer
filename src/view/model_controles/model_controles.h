#ifndef SRC_VIEW_MODEL_CONTROLES_H
#define SRC_VIEW_MODEL_CONTROLES_H
...
public:
    ...
    inline void Initialize(QSlider* ox_, QSlider* oy_, QSlider* oz_, QSlider* scale_){
    ...
    inline float GetRotateX(int position){return m_sl_x.ResetPosition(position);}
    inline float GetRotateY(int position){return m_sl_y.ResetPosition(position);}
    inline float GetRotateZ(int position){return m_sl_z.ResetPosition(position);}
    ...
    inline double GetScale(int position){return 1.0-0.1*static_cast<double>(m_scale.ResetPosition(position));}
...
#endif //SRC_VIEW_MODEL_CONTROLES_H